...
<template>
    <div class="space-y-4">
        <div class="relative">
            ...
            />
            <ul v-if="clientes.length" class="absolute z-10 mt-1 w-full rounded-lg border ">
                <li v-for="c in clientes" :key="c.id" @click="selecionarCliente(c)" class="cursor-pointer px-3 py-2 hover:">
                    {{ c.id }} — {{ c.nome }} — {{ c.email }}
                </li>
            </ul>
        </div>
