...
watch(
    ...
    (newItems: any) => {
        ...
        let payload: any = [];
        if (Array.isArray(newItems)) payload = newItems;
...
const userName = computed(() => String(page.props.auth?.user?.name ?? 'Usuário'));
...
const dashboardRoute = () => {
    Inertia.post(route('dashboardRota'));
}
                ...
                <div class="flex w-full items-center">
                    ...
                    <div
                        ...
                        :class="{
                            ...
                            'absolute top-full left-0 z-40 flex w-full flex-col items-center  py-4 text-center shadow-lg md:hidden':
                                isMenuOpen, // mostra no mobile com estilo
                        }"
                    >
                        <nav class="ml-4 flex flex-col gap-2 md:flex-row md:gap-4">
                            <Link :href="route('produtos.index')" @click="closeMenu" class="rounded px-4 py-2 hover:">Produtos</Link>
                            <Link href="/about-us" @click="closeMenu" class="rounded px-4 py-2 hover:">Quem Somos</Link>
                            <Link href="/how-to-buy" @click="closeMenu" class="rounded px-4 py-2 hover:">Como Comprar</Link>
                            <Link href="/contact" @click="closeMenu" class="rounded px-4 py-2 hover:">Contato</Link>
                            <Link href="#" @click="closeMenu" class="rounded px-4 py-2 hover:">Área do Produtor</Link>
                        </nav>
